...
use crate::color_utils::{
    color_distance, color_list_to_string, parse_color_string, rgb_to_hex,
};
...
static COLOR_FIELD_RE: Lazy<Regex> = Lazy::new(|| {
    Regex::new(
        r"(?im)(\b(m_ConstantColor|m_ColorFade|m_Color1|m_Color2|m_ColorTint|m_TintColor|m_ColorMin|m_ColorMax)\s*=\s*)(\[\s*[\d.,\s]+\s*\])",
    )
    .unwrap()
});
...
pub fn find_vpcf_files(folder: &str) -> Vec<String> {
    WalkDir::new(folder)
        ...
        .filter(|e| {
            e.path()
                .extension()
                .map_or(false, |ext| ext.eq_ignore_ascii_case("vpcf"))
        })
...
pub fn backup_file(path: &str) -> Result<(), String> {
    ...
    let bak_path = bak_dir.join(format!("{}.bak", filename.to_string_lossy()));
    if !bak_path.exists() {
...
pub fn parse_color_fields(content: &str, filename: &str) -> Vec<ColorField> {
    ...
    for g_match in GRADIENT_RE.find_iter(content) {
        ...
        let full_gradient_text = &content[g_match.start()..g_match.end()];
        let captures = GRADIENT_RE.captures(full_gradient_text).unwrap();
        let stops_body = captures.get(2).map_or("", |m| m.as_str());

        let mut stop_idx: i32 = 0;
        for s_match in STOP_RE.captures_iter(stops_body) {
            let full_stop = s_match.get(1).map_or("", |m| m.as_str()).to_string();
            let stop_pos = s_match.get(2).map_or("", |m| m.as_str()).to_string();
            let color_val = s_match.get(3).map_or("", |m| m.as_str()).to_string();

            fields.push(ColorField {
                field_type: "gradient".to_string(),
                value: color_val.clone(),
                full_match: full_stop.clone(),
                prefix: String::new(),
                field_name: format!(
                    "Gradient Block {} Stop {}",
                    gradient_block_counter,
                    stop_idx + 1
                ),
                raw_name: String::new(),
    ...
    for cap in COLOR_FIELD_RE.captures_iter(content) {
        ...
        *count += 1;
        let numbered_name = format!("{} {}", display_name, count);
...
pub fn apply_color_changes_to_content(
    ...
    // Apply non-gradient changes first
    for (&field_idx, new_rgb) in changes.iter() {
        ...
        let replacement = format!("{}{}", field.prefix, color_list_to_string(new_rgb));
        if let Some(pos) = new_content.to_lowercase().find(&search_str.to_lowercase()) {
            new_content = format!(
                "{}{}{}",
                &new_content[..pos],
                replacement,
    ...
    let gradient_changes: HashMap<(i32, i32), &Vec<u8>> = changes
        .iter()
        .filter_map(|(&idx, rgb)| {
            let f = &fields[idx];
    ...
    if !gradient_changes.is_empty() {
        ...
        for g_cap in GRADIENT_RE.captures_iter(&content_clone) {
            block_counter += 1;
            let full_match = g_cap.get(0).unwrap();
            result.push_str(&content_clone[last_end..full_match.start()]);
            ...
            let stops_body_clone = stops_body.to_string();

            for s_cap in STOP_RE.captures_iter(&stops_body_clone) {
                let key = (block_counter, stop_idx);
                if let Some(new_rgb) = gradient_changes.get(&key) {
                    let old_color_str = s_cap.get(3).map_or("", |m| m.as_str());
                    let new_color_str = color_list_to_string(new_rgb);
                    new_stops = new_stops.replacen(old_color_str, &new_color_str, 1);
                }
                stop_idx += 1;
            }
        ...
        result.push_str(&content_clone[last_end..]);
        new_content = result;
    }
...
pub fn compute_shared_colors(
    all_fields: &[ColorField],
    threshold: f64,
) -> Vec<SharedColorGroup> {
    struct GroupBuilder {
    ...
    for field in all_fields {
        ...
        let added = if let Some((idx, grp)) = closest_idx {
            if color_distance(&rgb, &grp.representative) <= threshold {
                groups[idx].field_names.insert(field.raw_name.clone());
        ...
        if !added {
            ...
            g.field_names.insert(field.raw_name.clone());
    ...
    let mut result: Vec<SharedColorGroup> = groups
        .into_iter()
        .filter(|g| g.total >= 2)
        .map(|g| {
...
pub fn apply_shared_color_to_files(
    ...
    for filename in filenames {
        ...
        let matching_fields: Vec<(usize, ColorField)> = file_data
            .fields
            .iter()
            .enumerate()
            .filter(|(_, f)| {
        ...
        let changes: HashMap<usize, Vec<u8>> = matching_fields
            .iter()
            .map(|(idx, _)| (*idx, new_rgb.to_vec()))
            .collect();

        let new_content =
            apply_color_changes_to_content(&file_data.content, &file_data.fields, &changes);
